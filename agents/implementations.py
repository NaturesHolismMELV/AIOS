...
===========================
Concrete agent implementations for the AIOS ecosystem.
Each agent has distinct φ, ε, domain characteristics.

Session 5 changes
─────────────────
- WritingAgent  → replaced by WriterAgent  (real Haiku LLM calls)
- PlannerAgent  → replaced by PlannerAgent (real Haiku LLM calls)
- Three-way token budget contention now live: ANALYSIS + WRITER + PLANNER
"""
...
# WritingAgent removed — replaced by WriterAgent (real Haiku LLM) in Session 5
# See agents/writer_agent.py
...
# PlannerAgent removed — replaced by PlannerAgent (real Haiku LLM) in Session 5
# See agents/planner_agent.py
...
def create_default_ecosystem(kernel: MELVKernel) -> dict:
    """
    Instantiate the default agent ecosystem.

    Session 5 change: WriterAgent and PlannerAgent are now real LLM agents.
    Three-way token_budget contention: ANALYSIS + WRITER + PLANNER.
    """
    agents = [
        ResearchAgent(kernel),
        AnalysisAgent(kernel),
        WriterAgent(kernel),    # real LLM — Session 5
        CodeAgent(kernel),
        MonitorAgent(kernel),
        PlannerAgent(kernel),   # real LLM — Session 5
        DataAgent(kernel),
        SearchAgent(kernel),
    ]
